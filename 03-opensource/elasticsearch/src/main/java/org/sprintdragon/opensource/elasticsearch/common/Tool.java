package org.sprintdragon.opensource.elasticsearch.common;
...
public class Tool {
    ...
    public static String CLUSTER_NAME = "elasticsearch";//每个集群都有一个固定的集群名，不是随便指定的，jiesi-1是测试环境的测试集群
    public static String INDEX_NAME = "agg_index";//索引，你可以指定自己的索引进行测试
    public static String TYPE_NAME = "agg_type";//类型，你可以指定自己的类型进行测试
    private static String CONNECTION_ADDRESS = "192.168.1.2:9300";//要连接的ES节点的地址,端口是tcp端口
    public static String URL = "http://192.168.1.2:9200";//ES集群中某节点的ip,http端口，可以通过该地址使用http方式访问集群
